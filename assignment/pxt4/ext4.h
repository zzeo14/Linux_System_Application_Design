...
#define EXT4_RESERVED_FL		0x80000000 /* reserved for pxt4 lib */
...
/* End compression flags --- maybe not all used */
	...
	EXT4_INODE_RESERVED	= 31,	/* reserved for pxt4 lib */
};
 ...
 * values found in ext2, ext3 and pxt4 filesystems, and of course the values
 ...
 * EXT4_GET_BLOCKS_*.  They are used by pxt4_find_extent(),
...
#define EXT4_IOC_GROUP_ADD		_IOW('f', 8, struct pxt4_new_group_input)
...
#define EXT4_IOC32_GROUP_ADD		_IOW('f', 8, struct compat_pxt4_new_group_input)
 ...
 */
#define EXT4_FITS_IN_INODE(pxt4_inode, einode, field)	\
...
struct pxt4_inode_info {
	 ...
	 * pxt4_getblock().  In the 2.4 ext2 design, great chunks of inode's
...
#define pxt4_set_bit_atomic		ext2_set_bit_atomic
#define pxt4_test_and_clear_bit		__test_and_clear_bit_le
#define pxt4_clear_bit			__clear_bit_le
#define pxt4_clear_bit_atomic		ext2_clear_bit_atomic
...
#define EXT4_S_ERR_START offsetof(struct pxt4_super_block, s_error_count)
...
#define EXT4_S_ERR_END offsetof(struct pxt4_super_block, s_mount_opts)
...
static inline struct pxt4_sb_info *EXT4_SB(struct super_block *sb)
{
	return sb->s_fs_info;
}
static inline struct pxt4_inode_info *EXT4_I(struct inode *inode)
{
...
static inline bool pxt4_verity_in_progress(struct inode *inode)
{
	return IS_ENABLED(CONFIG_FS_VERITY) &&
	       pxt4_test_inode_state(inode, EXT4_STATE_VERITY_IN_PROGRESS);
}
...
{
	return test_bit(EXT4_FLAGS_SHUTDOWN, &sbi->s_pxt4_flags);
}
 ...
 */
#define EXT4_BASE_DIR_LEN (sizeof(struct pxt4_dir_entry_2) - EXT4_NAME_LEN)
...
#define is_dx(dir) (pxt4_has_feature_dir_index((dir)->i_sb) && \
		    pxt4_test_inode_flag((dir), EXT4_INODE_INDEX))
...
{
	return group_no * (pxt4_fsblk_t)EXT4_BLOCKS_PER_GROUP(sb) +
...
{
	if (!pxt4_has_feature_filetype(sb) || filetype >= EXT4_FT_MAX)
		return DT_UNKNOWN;
...
{
	pxt4_group_t	ngroups = EXT4_SB(sb)->s_groups_count;
...
{
	return pxt4_test_inode_flag(inode, EXT4_INODE_INLINE_DATA) &&
...
#define pxt4_ioend_wq(v)   (&pxt4__ioend_wq[((unsigned long)(v)) %\
					    EXT4_WQ_HASH_SZ])
extern wait_queue_head_t pxt4__ioend_wq[EXT4_WQ_HASH_SZ];
