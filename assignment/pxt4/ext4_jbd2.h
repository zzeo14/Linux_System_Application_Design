...
static inline int pxt4_inode_journal_mode(struct inode *inode)
{
	...
	if (!S_ISREG(inode->i_mode) ||
	    ...
	    (pxt4_test_inode_flag(inode, EXT4_INODE_JOURNAL_DATA) &&
...
{
	return pxt4_inode_journal_mode(inode) & EXT4_INODE_JOURNAL_DATA_MODE;
}

static inline int pxt4_should_order_data(struct inode *inode)
{
	return pxt4_inode_journal_mode(inode) & EXT4_INODE_ORDERED_DATA_MODE;
}

static inline int pxt4_should_writeback_data(struct inode *inode)
{
	return pxt4_inode_journal_mode(inode) & EXT4_INODE_WRITEBACK_DATA_MODE;
}
